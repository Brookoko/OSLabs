...
{
    assert(total_size % block_alignment == 0 && "Total size should be 4 byte aligned");
...
{
    assert(validate_address(addr) && "Invalid address to reallocate");
    if (size <= 0)
    {
        mem_free(addr);
        return nullptr;
...
{
    assert(validate_address(addr) && "Invalid address to free");
    auto *header = static_cast<Header*>(addr) - 1;
    header->is_available = true;
    m_used -= header->size;
    defragment(header);
